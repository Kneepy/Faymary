...
import { ConnectedSocket, OnGatewayConnection, OnGatewayDisconnect, WebSocketGateway, WsException } from "@nestjs/websockets";
...
export class BaseGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleConnection(@ConnectedSocket() socket: ICustomSocket, ...[args]: [IncomingMessage]) {
        ...
        try {
            ...
            const user = await this.usersService.findOne({id: socket.id}, {relations: ["activity"]})

            if(user.activity) {
                user.activity = Object.assign(user.activity, {state: ActivityEnum.ONLINE, start: Date.now()})

                socket.state = user.activity
                await this.usersService.update(user)
            }
            else {
                socket.state = await this.activityService.create({user, state: ActivityEnum.ONLINE})
            }
        } catch (e) {
    ...
    async handleDisconnect(@ConnectedSocket() socket: ICustomSocket) {
        socket.state = Object.assign(socket.state, {state: ActivityEnum.OFFLINE, end: Date.now()})
