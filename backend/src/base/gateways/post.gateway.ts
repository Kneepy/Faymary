...
import { AddCommentToPostDto, AddLikeToPostDto } from "../dto/posts";
...
export class PostGateway {
    ...
    public async addCommentToPost(
        ...
        */
        ...
        const notification = await this.baseGateway.setNotification(NotificationEnumType.COMMENT, {user: post.user, sender: user}, post.user.settings.commentsOnPostNotifications)
    ...
    public async addLikeToPost(
        ...
        const user = await this.usersService.findOne({id: socket.id})
        const post = await this.postsService.findOne({id: body.postId}, {relations: ["likes", "user", "user.settings", "user.notifications", "user.notifications.sender"]})
        const like: Likes | any = {user}
        const postContainsLike = post.likes.findIndex(like => like.user.id === socket.id)
        ...
        if(postContainsLike !== -1) {
            ...
            */
            ...
            const notification = await this.baseGateway.setNotification(NotificationEnumType.LIKE_POST, {user: post.user, sender: user}, post.user.settings.likeOnPostNotifications)
        ...
        const updatedPost = await this.postsService.update(post)
        const creatorPostSocket = await this.baseGateway.findUser(updatedPost.user.id)
