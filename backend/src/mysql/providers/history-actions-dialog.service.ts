...
export class HistoryActionsService {
    constructor(@InjectRepository(HistoryActions) private repository: Repository<HistoryActions>) {}

    public async find(args: ManyRelationshipsArgs, options?: FindManyOptions<HistoryActions>): Promise<HistoryActions[]> {
        return await this.repository.find({where: args, ...options})
    }

    public async findOne(args: RelationshipsArgs, options?: FindOneOptions<HistoryActions>): Promise<HistoryActions> {
