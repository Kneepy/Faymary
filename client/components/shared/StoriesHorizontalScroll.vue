...
const visibleItemsBox = ref<HTMLElement>()
const shiftItems = ref(0)

const addShiftItems = (direction: number, skipPack: boolean = false) => {
    console.log(itemsBox.value?.offsetWidth, itemsBox.value?.clientWidth, visibleItemsBox.value?.offsetWidth)
    ...
    console.log(widthVisibleStories)
...
const scrollItems = (e: WheelEvent) => addShiftItems([0, -0].includes(-e.deltaY) ? -e.deltaX : -e.deltaY, false)
...
<template>
    <div ref="visibleItemsBox" @wheel="scrollItems" class="horizontal-scroll">
        ...
        />
        <div ref="itemsBox" class="scroll-items">
            <slot :scrollPosition="position" :scroll="addShiftItems" :shift="shiftItems" />
        </div>
    </div>
</template>
