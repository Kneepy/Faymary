...
export class MessagesGateway {
    ...
    async createMessage(@MessageBody() data: BrokerRequests.Message.Create, @ConnectedSocket() client: ICustomSocket): Promise<void> {
        try {
            const has_attachments = Object.keys(data.attachments).length > 0;
            ...
            const attachments = await this.attachmentsProvider.setAttachments({
    ...
    async addReactionMessage(@MessageBody() {
        unity,
        message_id
    }: BrokerRequests.Message.AddReaction, @ConnectedSocket() client: ICustomSocket): Promise<void> {
        ...
        if (!collection) {
            ...
            like = await this.likesService.addLike({ user_id: client.user_id, collection_id: newCollection.id }).toPromise()
            ...
            like = await this.likesService.addLike({ user_id: client.user_id, collection_id: collection.id }).toPromise()
